...
func (t *LoyaltyChaincode) Init(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	t.addProduct(stub, []string{"Speakers", "2000", "200", "AcmeMart", "100"})
	t.addProduct(stub, []string{"Headphones", "3000", "300", "AcmeMart", "100"})
	t.addProduct(stub, []string{"Mobile", "4000", "400", "AcmeMart", "100"})
	t.addProduct(stub, []string{"BagPack", "1000", "100", "AcmeMart", "100"})
...
/*func (t *LoyaltyChaincode) addBalance(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {

	fmt.Println("addBalance is running ")

	if len(args) != 2 {
		return nil, errors.New("Incorrect Number of arguments.Expecting 2 for add")
	}

	key := args[0]                              //custName
	amt, err := strconv.ParseFloat(args[1], 64) // balance to be added
	...
	fmt.Println(customer.Points)

	// Perform the addition of assests
	customer.Balance = customer.Balance + amt
	fmt.Println("customer Balance = ", customer.Balance)
